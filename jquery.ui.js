	...
	if( $obj == 'basic' ){
		...
		$('.lnb_basic .lnb_link').click(function(e){
			...
			if ( $list.hasClass('open') ){
				...
				// $list.find('.lnb_link').removeClass('selected'); // hwshim_20191031	--> jbsun_20200309
				...
				$list.addClass('open');
				// $list.find('.lnb_link').addClass('selected'); // hwshim_20191031	--> jbsun_20200309
...
function lnbFold(ani, lnbW, lnbMargin, status, btnTop) {
	...
	if(status == true){
		...
		//혜미 - false slide일때 설정
		if(ani == 'slide'){
			//$('.btn_folder_wrap').css('top', '20px');
			$("#lay_contents").css('marginLeft', '228');
		};
	};
